...
public class MoneySerializer2ManualVersionTest {
  ...
  public void testCurrent() throws IOException, SAXException {
    ...
    AssertUtils.assertXMLEqual( out.toString(), "<money xmlns=\"http://www.cedarsoft.com/test/money/1.0.1\" cents=\"799\" />" );
  ...
  public void testOldFormat() throws IOException {
    MoneySerializer2 serializer = new MoneySerializer2();
    assertEquals( serializer.deserialize( new ByteArrayInputStream( ( "<money xmlns=\"http://www.cedarsoft.com/test/money/1.0.0\">799</money>" ).getBytes() ) ), new Money( 7, 99 ) );
  }

  @Test
  public void testCurrentFormat() throws IOException {
    MoneySerializer2 serializer = new MoneySerializer2();
    assertEquals( serializer.deserialize( new ByteArrayInputStream( ( "<money xmlns=\"http://www.cedarsoft.com/test/money/1.0.1\" cents=\"799\" />" ).getBytes() ) ), new Money( 7, 99 ) );
  }
}
